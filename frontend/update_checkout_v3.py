...
content = content.replace(payload_fallback, payload_new)
...
# 6. Replace Using Split Blocks (No Regex!)
target_start = "{/* ── NEQUI (REALISTA) ── */}"
target_end = "{/* ── TARJETA SIMULADA (UI REALISTA) ── */}"

if target_start in content and target_end in content:
    parts = content.split(target_start)
    before_nequi = parts[0]
    rest = parts[1]
    
    parts2 = rest.split(target_end)
    after_card = parts2[1]
    
    new_gateways = '''{/* ── NEQUI (GATEWAY REAL) ── */}
    ...
    content = before_nequi + new_gateways + after_card
else:
    print("Could not find targets for blocks!")
